function [estFin, optFin, estHist, state] = ...
    BiGAMP_matrix(gX, gA, gOut, problem, opt, state)
...
while ~stop
    ...
    if ~uniformVariance
        ...
        %Compute zvar
        if ~sparseMode
            Avar = reshape(Avar, [M,N]);
            Ahat2 = reshape(Ahat2, [M,N]);
