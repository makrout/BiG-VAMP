...
if checkA
    if verbose
        fprintf('matrix normalization check: ')
    end
...
while ~stop
    ...
    % Compute transform output estimate
    Axhat = A.mult(xhat);
      ...
      switch rvarMethod
        ...
        case 'median' 
          if any(~isreal(vhat))
            ...
            % since inv_cdf_norm(3/4) ~= 0.6745 ...
            rvar = ((1/0.6745)*median(abs(vhat),1)).^2; 
          end
        case 'wvar' 
          rvar = (scale^2)*wvar + (n/m)*xvar;
      end
